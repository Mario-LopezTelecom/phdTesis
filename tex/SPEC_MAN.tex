...
A central issue in OSA is how to preserve the quality of service (QoS) experienced by PUs while offering a reasonable grade of service to SUs.
...
Third, because of the scanning delay, using many scanning slots implies \textit{more uncertainty about the information obtained in earlier slots}.
In a real system, where sensing results are not fully reliable, this uncertainty is even higher.
This last aspect has been largely ignored in the existing literature, and is the first key issue addressed in this paper. 

The second key issue is to evaluate the impact of the channel allocation scheme for PUs on OSA performance.
...
This paper continues the line initiated in \cite{ref:Alcaraz2013}, where we studied the impact of PU bandwidth reservation on OSA performance. 
The results in \cite{ref:Alcaraz2013} showed that the benefits of reduced PU-SU collisions outweigh the negative consequences of a more limited selection of channels available to the PUs. 
...
Fewer works \cite{ref:Huang2008_opp} consider overlapping time which,
...
\begin{enumerate}
\item We develop an optimal SU access strategy that considers the effect of scanning delay in the uncertainty of the information available at the SUs. This effect has been largely ignored in the design of OSA protocols for hardware limited radios. \item The OSA scheme is designed and evaluated for different levels of \textit{coexistence friendliness} at the spectrum owner, reflected on its spectrum management strategies.
\end{enumerate}
...
Designing the optimal OSA protocol implies solving a Partially Observable Markov Decision Process (POMDP) with finite horizon. This is a challenging task because of the inherent complexity of POMDPs, which is increased by adding the effect of the scanning delay on the reliability of past observations. As we show, the spectrum management policy of the primary operator plays a crucial role on this effect.
...
%The SUs implement the MAC protocol (HC-MAC) described in \cite{ref:Jia2008} with a different sensing and accessing decision policy. 
