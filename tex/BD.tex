...
\caption{Integration regions to compute the probabilities of correct detection, type I and type II errors under the H1 event (transmission overlap).}\label{fig:Regions}
\end{figure}
...
We also assume that it takes certain number of slots $n_s$ for the SU to detect at least one available channel. For the sake of simplicity, we will consider $n_s$ fixed to 5, although, as we explain letter, the resulting expressions can be easily generalized for the case of a randomly distributed $n_s$.
